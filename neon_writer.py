...
try:
    ...
    # Neon is a shadow write and Sheets stays authoritative, so a host that
    # has not installed psycopg must still be able to run a scrape. Failing
    # here would abort at import — scrape_orders imports writers, which
    # imports this module — and take the whole run down for the sake of an
    # optional mirror. Degrade to Sheets-only instead; _get_pool() returns
    # None below and every writer no-ops through _guard, exactly as it does
...
def _get_pool():
    ...
    if ConnectionPool is None:
        if not _warned_no_url:
            print(f"ℹ️  neon: psycopg not installed ({_IMPORT_ERROR}) — skipping Neon writes")
            _warned_no_url = True
        return None
    with _pool_lock:
        if _pool is not None:
